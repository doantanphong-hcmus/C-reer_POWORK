...
* **Bảng `Challenges`:** Lưu trữ thông tin tổng quan của thử thách (`title`, `description`, `deadline`). Nó lưu `company_id` dưới dạng một trường dữ liệu thông thường để biết thử thách thuộc về doanh nghiệp nào, hoàn toàn không JOIN cứng với module IAM.
...
* **Bảng Công khai `Submissions`:**
    ...
    * Bảng này **tuyệt đối không có cột `user_id`**. Danh tính ứng viên chỉ được thể hiện qua chuỗi mã hóa `hash_id`.
...
* **Bảng `Verified_Evidences`:**
    ...
    * Bảng này lưu `user_id` để biết hồ sơ thuộc về ai. Tuy nhiên, nó áp dụng chiến lược **Snapshot Data**: copy trực tiếp chuỗi văn bản tĩnh như `challenge_name` và `company_name` tại thời điểm mở khóa để lưu trữ.
...
4.  Hệ thống thực hiện 2 thao tác Insert ghi nhận dữ liệu:
    ...
    * Ghi thông tin bài nộp vào bảng công khai `Submissions` với khóa định danh duy nhất là `hash_id` (Hoàn toàn sạch bóng thông tin `user_id`).
...
3.  Nếu bài làm xuất sắc, nhà tuyển dụng bấm nút **"Duyệt & Mở khóa" (Approve & Unlock)**:
    ...
    * Đồng thời, một Event được kích hoạt, truyền `user_id`, tên bài toán và tên công ty sang **Profile Module** để tự động ghi nhận một bản ghi năng lực mới vào bảng `Verified_Evidences`.
