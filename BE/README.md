...
├── src/
│   ├── config/
│   │   ├── index.js               ← Biến cấu hình toàn app (port, jwt, ...)
│   │   └── prisma.js              ← Khởi tạo kết nối Database (singleton)
│   │
│   ├── controllers/               ← Nhận request, gọi service, trả response
│   │   ├── auth.controller.js     ← IAM Module: login, register, getMe
│   │   ├── challenge.controller.js← Challenge Module: CRUD challenge + rubrics
│   │   ├── submission.controller.js← Assessment Module: nộp bài, chấm, unlock
│   │   └── profile.controller.js  ← Profile Module: Dynamic Profile công khai
│   │
│   ├── middlewares/
│   │   ├── auth.middleware.js     ← Xác thực JWT + Blind Audition Guard
│   │   └── error.middleware.js    ← Bắt lỗi tập trung cho toàn app
│   │
│   ├── routes/                    ← Định tuyến request vào đúng controller
...
│   │
│   ├── utils/
│   │   ├── AppError.js            ← Class lỗi tùy chỉnh có statusCode
│   │   ├── hashId.js              ← Sinh mã ẩn danh Candidate_XXXX
│   │   └── response.js            ← Chuẩn hóa format JSON trả về
│   │
│   ├── app.js                     ← Cấu hình Express (gắn middleware, route)
│   └── server.js                  ← Điểm khởi động server
│
├── .env.example                   ← Mẫu biến môi trường (copy thành .env)
├── docker-compose.yml             ← Khởi động PostgreSQL bằng Docker
├── package.json                   ← Danh sách thư viện và scripts
├── .prettierrc                    ← Quy tắc format code
└── eslint.config.js               ← Quy tắc lint code
```

> **Sprint 1+ sẽ bổ sung:** `services/`, `repositories/`, `models/`  
> Sprint 0 đã có đủ `controllers/` — toàn bộ logic hiện dùng Mock Data, chờ Sprint 1 kết nối DB thật.
...
### `src/controllers/*.controller.js` — Điều phối viên

Mỗi controller tương ứng với 1 module nghiệp vụ. Nhiệm vụ duy nhất của controller là:
...
Controller **không được** tự query DB, không chứa logic nghiệp vụ, không tự catch lỗi. 
...
router.post('/submissions', authenticate, authorize('CANDIDATE'), blindAuditionGuard, submitSolution)
...
{
  "evaluations": [
    { "criteria_id": "aa152d43-...", "score": 8.5, "comment": "Tốt" }
  ],
